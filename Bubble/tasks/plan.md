...
Bubble 是一款 macOS 菜单栏提示词管理工具，用户通过状态栏图标或全局快捷键唤起实色玻璃风格面板，对提示词进行增删改查、分类筛选、拖动排序和一键复制。面板可移动，并能轻微吸附到屏幕可用边缘。

## 技术栈

| 层级 | 技术选型 | 说明 |
|------|----------|------|
| UI 框架 | SwiftUI + AppKit | SwiftUI 声明式 UI；AppKit 管理菜单栏、自定义 NSPanel、窗口移动与吸附 |
...
class Prompt {
    var id: UUID
    var title: String           // 标题
    var content: String         // 提示词内容（最大 5000 字符）
...
- 创建自定义 NSPanel（无标题栏、圆角实色玻璃背景）
- 面板首次从当前屏幕右上方弹出，尺寸约 420×520pt
- 点击面板外部自动关闭（`hidesOnDeactivate`）
- 嵌入 SwiftUI 视图（NSHostingView）
- **验收**：点击图标弹出实色玻璃空面板，点外部消失

> **Checkpoint 1**：菜单栏图标可点击，弹出实色玻璃空面板，右键菜单可用。
...
- 复制后按钮短暂变为 checkmark 反馈，随后自动收起面板
...
- 表单字段：标题输入框、提示词内容（多行 TextEditor，5000 字符上限 + 字数统计）、标签选择（可选，彩色标签按钮）
- 保存/取消按钮
- 保存时校验：标题和内容不能为空
...
#### Task 6.1: 实色玻璃与视觉打磨
- NSPanel 使用不透明实色容器，避免后方窗口透出影响阅读与拖动
- SwiftUI 层面使用柔和渐变、高光和颗粒保留玻璃质感
- 面板圆角（16pt）、阴影
- 标签栏颜色与原型图一致（蓝/绿/橙/粉/紫/黄/红/灰）
- 卡片 hover 效果、按钮点击动画
- **验收**：整体视觉与原型图一致，背景不透出，卡片和标签清晰可读
...
**目标**：让长列表整理与浮窗摆放更直接，同时保证提示词数据安全。

#### Task 7.1: 面板移动与轻吸附
- 仅顶部标题区域负责窗口拖动，避免抢占卡片手势
- 第一次弹出时定位到当前屏幕右上方
- 拖动中在 18pt 范围提供轻磁力，松手后在 28pt 范围完成吸附
- 使用 `NSScreen.visibleFrame` 避开菜单栏和 Dock，多屏时跟随鼠标所在屏幕
- **验收**：面板可自由移动，靠近四边时有轻微吸附，且不会覆盖菜单栏或 Dock

#### Task 7.2: 卡片即时拖动排序
- 按住卡片并移动约 3pt 即进入排序，无需静止等待
- 拖动中的卡片和预览缩小至 80%，并保持在列表区域内
- 在两张卡片之间绘制蓝色横向插入提示，松手后立即清理
- 只更新现有 Prompt 的 `sortOrder` 并保存，不删除或重建提示词
- **验收**：快速拖动可用，排序持久化，提示词数量与内容不变

#### Task 7.3: 长列表边缘自动滚动
- 列表顶部和底部各设置 64pt 连续滚动热区
- 拖入热区后按固定节奏滚动，不积压动画任务
- 到达列表边界或拖动结束时立即停止
- **验收**：末尾卡片可以连续拖到列表首部，应用不会卡住或残留拖动预览

> **Checkpoint 7**：面板移动吸附与卡片长列表排序已完成，并通过真实拖动验证。
